...
# #################################
df = pd.read_csv(
    "https://docs.google.com/spreadsheets/d/15HoJRe3AGq3VAgXN8gcO3cF0vfezPR6t0LvTc2R4WFo/export?format=csv&gid=896724717",
    parse_dates=['DATA'],
    date_format='%d/%m/%Y'
)
...
df = df[
    (df['DATA'] >= pd.to_datetime(st.session_state.date_from)) &
    (df['DATA'] <= pd.to_datetime(st.session_state.date_to))
]
...
df.loc[
    (df['TIPOLOGIA'] == 'SMART WORKING') |
    (df['TIPOLOGIA'] == 'PESCARA') |
    (df['TIPOLOGIA'] == 'TERAMO') |
    (df['TIPOLOGIA'] == 'MISSIONE'),
    'DOVUTO GIORNALIERO'
] = pd.to_timedelta(25920, unit='s')
...
def calcola_ore_lavorate(row):
    ...
    ore_lavorate = (
        pd.to_datetime(row['USCITA_1'], format="%H:%M") -
        pd.to_datetime(row['ENTRATA_1'], format="%H:%M")
    )

    # Se 'ENTRATA_2' e 'USCITA_2' sono valorizzate, somma anche queste
    if not pd.isna(row['ENTRATA_2']) and not pd.isna(row['USCITA_2']):
        ore_lavorate += (
            pd.to_datetime(row['USCITA_2'], format="%H:%M") -
            pd.to_datetime(row['ENTRATA_2'], format="%H:%M")
        )

    # Se 'PAUSA' è valorizzata, sottrai la pausa
...
df.loc[
    ...
    (df['TIPOLOGIA'] == 'EVENTO'),
    'ORE LAVORATE'
] = df.apply(calcola_ore_lavorate, axis=1)
...
ddf = df[['DATA', 'GIORNO', 'SETTIMANA', 'TIPOLOGIA', 'DETTAGLI',
          'ORE RICHIESTE', 'ENTRATA_1', 'ENTRATA_2',
          'USCITA_1', 'USCITA_2', 'PAUSA',
...
def row_color(row):
    ...
    elif row['TIPOLOGIA'] in ['PERMESSO', 'VISITA MEDICA', 'RECUPERO ORE RICERCATORI']:
...
st.dataframe(
    ...
    column_config={
        ...
        "SALDO_GIORNALIERO_FORMATTED": st.column_config.Column(label="SALDO GIORNALIERO"),
        "DOVUTO_GIORNALIERO_FORMATTED": st.column_config.Column(label="DOVUTO GIORNALIERO"),
        "ORE_LAVORATE_FORMATTED": st.column_config.Column(label="ORE LAVORATE"),
    },
...
with st.container(border=True):
    ...
    st.metric(
        label="Saldo Generale (hh:mm)",
        value=str(format_saldo(saldo_generale)).split(":")[0] +
        " ore e " +
        str(format_saldo(saldo_generale)).split(":")[1] +
        " minuti"
    )
...
with col1:
    with st.container(border=True):
        ...
        with tab1:
            df_settimanale['SALDO_GIORNALIERO_ORE'] = (
                df_settimanale['SALDO GIORNALIERO'].dt.total_seconds() / 3600
            )
            df_settimanale['SALDO_GIORNALIERO_FORMATTED'] = (
                df_settimanale['SALDO GIORNALIERO'].apply(format_saldo)
            )
            colors = [
                'mediumseagreen' if x >= 0 else 'firebrick'
                for x in df_settimanale['SALDO_GIORNALIERO_ORE']
            ]
        ...
        with tab2:
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "SALDO_SETT_FORMATTED": st.column_config.Column(
                        label="SALDO SETTIMANALE (Ore:Minuti)"
                    )
                }
            )
...
with col2:
    with st.container(border=True):
        ...
        with tab1:
            df_mensile['SALDO_GIORNALIERO_ORE'] = (
                df_mensile['SALDO GIORNALIERO'].dt.total_seconds() / 3600
            )
            df_mensile['SALDO_GIORNALIERO_FORMATTED'] = (
                df_mensile['SALDO GIORNALIERO'].apply(format_saldo)
            )
            colors = [
                'mediumseagreen' if x >= 0 else 'firebrick'
                for x in df_mensile['SALDO_GIORNALIERO_ORE']
            ]
        ...
        with tab2:
            st.dataframe(
                df_mensile[['MESE', 'SALDO_MENSILE_FORMATTED']],
                hide_index=True,
                use_container_width=True,
                column_config={
                    "SALDO_MENSILE_FORMATTED": st.column_config.Column(
                        label="SALDO MENSILE (Ore:Minuti)"
                    )
                }
            )
